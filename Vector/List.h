...
//템플릿 동적 배열 클래스
//템플릿은 왜 해더와 cpp로 구분해서 구현하지 않는가? -> 템플릿은 헤더에만 구현할 수 있음. 
//템플릿은 추론 시점이 컴파일 시간이기 때문. .cpp는 링커랑 연결해서 해석하기 때문.                        
template<typename T>
class List
{
public:
	...
	void push_back(T&& value) // Rvalue 참조
	{
		if (size == capacity)
		{
			int newCapacity = capacity == 0 ? defaultSize : capacity * scale;
